...
public class DECtoZSC {
	...
	private static void replaceWord(StringBuilder stringBuilder, String oldWord, String newWord) {
		if(stringBuilder.toString().toUpperCase().contains(oldWord)) {
			stringBuilder.replace(
					stringBuilder.toString().toUpperCase().indexOf(oldWord.toUpperCase()),
					stringBuilder.toString().toUpperCase().indexOf(oldWord.toUpperCase())+oldWord.length(),
					newWord);
		}
	}
		...
		while (dec.hasNextLine()) { // Go through this line by line
			...
			if(insideDefaults) {
				if(line.toUpperCase().contains("STATES") || line.toUpperCase().contains("}")) {
					...
					// TODO: This is where you could and probably should dump the VARs
			...
			// FIX LACK OF QUOTATION MARKS
			if(!line.contains("\"")) {
				// RenderStyle
				if(line.toUpperCase().contains("RENDERSTYLE") &! line.contains("STYLE_")) {
				//	zline.replace(line.toUpperCase().indexOf("RENDERSTYLE "),
				//			line.toUpperCase().indexOf("RENDERSTYLE ")+12,
				//			"RenderStyle \"");
					replaceWord(zline, "NONE", "\"None\"");
					replaceWord(zline, "NORMAL", "\"Normal\"");
					replaceWord(zline, "FUZZY", "\"Fuzzy\"");
					replaceWord(zline, "SOULTRANS", "\"SoulTrans\"");
					replaceWord(zline, "OPTFUZZY", "\"OptFuzzy\"");
					replaceWord(zline, "STENCIL", "\"Stencil\"");
					replaceWord(zline, "ADDSTENCIL", "\"AddStencil\"");
					replaceWord(zline, "TRANSLUCENT", "\"Translucent\"");
					replaceWord(zline, "ADD", "\"Add\"");
					replaceWord(zline, "SUBTRACT", "\"Subtract\"");
					replaceWord(zline, "SHADED", "\"Shaded\"");
					replaceWord(zline, "ADDSHADED", "\"AddShaded\"");
					replaceWord(zline, "SHADOW", "\"Shadow\"");
				}
			}
			...
			// Last Moment Cleaning
			if(zline.lastIndexOf(") ()") > 0) zline.delete(zline.lastIndexOf(") ()")+1, zline.lastIndexOf(") ()")+4);
			if(zline.lastIndexOf(" ()") > 0) zline.deleteCharAt(zline.lastIndexOf(" ()"));
			if(zline.lastIndexOf(" ;") > 0) zline.deleteCharAt(zline.lastIndexOf(" ;"));
			if(zline.lastIndexOf(";;") > 0) zline.deleteCharAt(zline.lastIndexOf(";;"));
